...
class Agent:
    def __init__(
        ...
        self.actions_history = (
            None  # action history vector as record, later initialized in train/predict
        )
        self.steps_done = (
            0  # to count how many steps it used to compute the final bdbox
        )
    ...
    def save_network(self):
        torch.save(
            self.policy_net,
            self.save_path + "_" + self.model_name + "_" + self.curr_class,
        )
        print("Saved")
    ...
    def compute_reward(self, actual_state, previous_state, ground_truth_boxes, depths):
        """
        Compute the reward based on IoU before and after an action (not trigger).
        If multiple ground truth boxes have the same maximum IoU, randomly choose one based on depth.
        The reward will be +1 if IoU increases, and -1 if decreases or stops.
        ----------
        Argument:
        actual_state   - new bounding box after action
        previous_state - old bounding box
        ground_truth_boxes - list of ground truth bounding boxes of current object
        depths         - list of depths corresponding to each ground truth bounding box
        *all bounding boxes come in four elements vector (left, right, top, bottom)
        ----------
        Return:
        ...
        for gt_box, depth in zip(ground_truth_boxes, depths):
            iou = self.intersection_over_union(actual_state, gt_box)
            if iou > max_iou:
                max_iou = iou
                candidates = [(gt_box, depth)]
            elif iou == max_iou:
                candidates.append((gt_box, depth))

        if candidates:
            chosen_gt, chosen_depth = random.choice(candidates)
        else:
            chosen_gt = ground_truth_boxes[0]

        res = self.intersection_over_union(actual_state, chosen_gt) - \
              self.intersection_over_union(previous_state, chosen_gt)

        if res <= 0:
            return -1
        return 1

    def compute_trigger_reward(self, actual_state, ground_truth_boxes, depths):
        """
        ...
        for i in range(0, self.n_actions):
            ...
            if i != 0:
                reward = self.compute_reward(
                    new_equivalent_coord, current_coord, ground_truth
                )
            else:
    ...
    def optimize_model(self, verbose):
        ...
        # fetch action_batch
        action_batch = Variable(torch.LongTensor(batch.action).view(-1, 1)).type(
            LongTensor
        )
        # fetch reward_batch
        ...
        for i_episode in range(self.num_episodes):
            ...
            for key, value in train_loader.items():
                ...
                if verbose:
                    img_id += 1
                    print(
                        "Training on Img {}/{}".format(
                            img_id, len(train_loader.items())
                        )
                    )
                ...
                while not done:
                    ...
                    if action == 0:
                        next_state = None
                        closest_gt = self.get_max_bdbox(
                            ground_truth_boxes, self.current_coord
                        )
                        reward = self.compute_trigger_reward(
                            self.current_coord, closest_gt
                        )
                        done = True
                    ...
                    # if not, compute next coordinate
                        ...
                        new_equivalent_coord = self.calculate_position_box(
                        ...
                        closest_gt = self.get_max_bdbox(
                            ground_truth_boxes, new_equivalent_coord
                        )
                        reward = self.compute_reward(
                            new_equivalent_coord, self.current_coord, closest_gt
                        )
    ...
    def predict_image(self, image, plot=False, verbose=False):
        ...
        while not done:
            ...
            if verbose:
                print(
                    "Iteration:{} - Action:{} - Position:{}".format(
                        steps, action, new_equivalent_coord
                    )
                )
            ...
            if plot:
                show_new_bdbox(
                    original_image, new_equivalent_coord, color="b", count=steps
                )
    ...
    def predict_multiple_objects(self, image, plot=False, verbose=False):
        ...
        while 1:
            ...
            if cross_flag:
                ...
                mask[
                middle_y - length_y: middle_y + length_y,
                int(bdbox[0]): int(bdbox[1]),
                ] = 0
                mask[
                int(bdbox[2]): int(bdbox[3]),
                middle_x - length_x: middle_x + length_x,
                ] = 0
...
class Agent_PPO(Agent):
   def __init__(self, curr_class, nu=3.0, threshold=0.5, num_episodes=15, load=False, model_name='vgg16'):
    super().__init__(curr_class=curr_class, nu=nu, threshold=threshold, num_episodes=num_episodes, load=load, model_name=model_name)
    if not load:
      self.policy_net = PPO()
    else:
