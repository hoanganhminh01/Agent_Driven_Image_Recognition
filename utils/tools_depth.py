...
def sort_class_extract(datasets, classes):
    """
    Change a whole dataset to separate datasets corresponding to classes variable, including depth information.
    """
    datasets_per_class = {}
    for j in classes:
            ...
            if isinstance(obj, list):
                for j in range(len(obj)):
                    ...
                    if curr_class in classes:
                        org[curr_class].append([obj[j]["bndbox"], target['annotation']['size'], obj[j].get("depth", None)])
            else:
                if curr_class in classes:
                    org[curr_class].append([obj["bndbox"], target['annotation']['size'], obj.get("depth", None)])
            for j in classes:
                if len(org[j]) > 1:
                    try:
...
def extract(index, loader):
    ...
    for ex in extracted[0][1:]:
        ...
        depths.append(depth)
    return img, ground_truth_boxes, depths
