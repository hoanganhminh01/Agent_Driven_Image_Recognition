...
class Agent():
    def __init__(self, curr_class, alpha=0.2, nu=3.0, threshold=0.5, num_episodes=15, load=False, model_name='vgg16', n_actions=9):
        # basic settings
    ...
    def save_network(self):
        torch.save(self.policy_net, self.save_path + "_" + self.model_name + "_" +self.curr_class)
        print('Saved')

    def load_network(self):
        if not use_cuda:
            return torch.load(self.save_path + "_" + self.model_name + "_" + self.curr_class, map_location=torch.device('cpu'))
        return torch.load(self.save_path + "_" + self.model_name + "_" + self.curr_class)
...
class Agent_3alpha(Agent):
  def __init__(self, curr_class, alpha=0.2, nu=3.0, threshold=0.5, num_episodes=15, load=False, model_name='vgg16', n_actions=25):
    super().__init__(curr_class=curr_class, alpha=alpha, nu=nu, threshold=threshold, num_episodes=num_episodes, load=load, model_name=model_name, n_actions=n_actions)
